...
Current production scope: coordinator-led Telegram control plane, one active worker, real OpenRouter pipeline, SQLite-backed state, project-aware execution resolved from bound project chats or the single-project owner-DM/bootstrap fallback, and an entrypoint that can launch multiple Telegram bot identities from `TELEGRAM_AGENT_TOKENS` with role-aware outbound sending plus coordinator fallback when a specific identity is unavailable. Bound project chats now receive agent lifecycle progress from the matching bot identity when that sender exists, and owner private DMs can stay on the exact bot identity thread the owner used.
