...
- `L0.6` — first live roster expansion / role-aware multi-agent Telegram loop
  (`docs/LOCAL_MULTI_AGENT_TELEGRAM_LOOP.md`)
- `L0.7` — first successful live Telegram task loop / owner-facing recovery
...
- `L0.18` — connect the real `TELEGRAM_DATA_BOT_TOKEN` and certify the first
  personal live DM proof for `data_agent`
  (`docs/LOCAL_DATA_AGENT_LIVE_IDENTITY.md`)
...
5. **L0.1-L0.7** — isolated local pilot track без Docker, с first real local
   Telegram contour, first bounded multi-agent task loop, owner-facing recovery
   path и без прямого attach к live main project.
