...
def test_build_running_multi_bot_runtime_accepts_promoted_data_identity(
    tmp_path,
):
    built_tokens: list[str] = []
    ...
    assert runtime is not None
    assert built_tokens == ["123:coord", "779:data"]
    assert tuple(runtime.applications_by_role.keys()) == (
        COORDINATOR_ROLE,
        "data_agent",
    )
    ...
    assert (
        runtime.lifecycle_report.states_by_role["data_agent"].application_built
        is True
    )
    assert len(runtime.applications_by_role["data_agent"].application.handlers) == 1
